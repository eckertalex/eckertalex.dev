...
export async function http<ResponseType>(request: RequestInfo): Promise<HttpResponse<ResponseType>> {
...
export async function get<ResponseType>(path: string, args?: RequestInit): Promise<HttpResponse<ResponseType>> {
...
export async function post<RequestBody, ResponseType>(
  ...
  const init = {method: 'post', body: typeof body === 'string' ? body : JSON.stringify(body), ...args}
  return await http<ResponseType>(new Request(path, init))
}
