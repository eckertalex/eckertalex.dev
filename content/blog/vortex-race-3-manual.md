---
{
    ...
    "description": "Manual for the Vortex Race 3, nearly as compact as a 60% board but with close to a TKL's key count.",
...
- Press the <kbd>Fn</kbd> + <kbd>R-Ctrl</kbd> keys. (Solid white led signals mapping mode active)
- Press the target <kbd>key(s)</kbd> to be re-mapped. (Flashing red led signals start of mapping)
...
- Press and hold the <kbd>Fn</kbd> + <kbd>L-Alt</kbd> keys for 3 seconds, then release
...
- Press and host the <kbd>Fn</kbd> + <kbd>r</kbd> keys for 5 seconds, then release
- The original <kbd>Fn</kbd> key can also be used even if it has been re-mapped

##### All Layers

- Press and hold the <kbd>L-Alt</kbd> + <kbd>R-Alt</kbd> keys for 5 seconds, then release
