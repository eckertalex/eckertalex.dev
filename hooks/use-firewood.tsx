...
export function useFirewood(initialValues: { solidMeter: string }) {
	const { value: solidMeter, set: setSolidMeter } = useLocalStorageValue(
		'calc/firewood__solidMeter',
		{
			defaultValue: initialValues.solidMeter,
		},
	)
	const { value: piledMeter, set: setPiledMeter } = useLocalStorageValue(
		'calc/firewood__piledMeter',
		{ defaultValue: solid2piled(initialValues.solidMeter) },
	)
	const { value: dumpingVolume, set: setDumpingVolume } = useLocalStorageValue(
